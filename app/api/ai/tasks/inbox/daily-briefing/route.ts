...
export async function POST(req: Request) {
  try {
    ...
    const enabled = await isAIFeatureEnabled(organizationId, 'ai_daily_briefing');
    if (!enabled) {
    ...
    const resolved = await getResolvedPrompt(organizationId, 'task_inbox_daily_briefing');
