...
export async function POST(req: Request) {
  try {
    ...
    const enabled = await isAIFeatureEnabled(null, organizationId, 'ai_daily_briefing');
    if (!enabled) {
    ...
    const resolved = await getResolvedPrompt(null, organizationId, 'task_inbox_daily_briefing');
