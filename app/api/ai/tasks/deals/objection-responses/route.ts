...
export async function POST(req: Request) {
  try {
    ...
    const enabled = await isAIFeatureEnabled(null, organizationId, 'ai_objection_responses');
    if (!enabled) {
    ...
    const resolved = await getResolvedPrompt(null, organizationId, 'task_deals_objection_responses');
