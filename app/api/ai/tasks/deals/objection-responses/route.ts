...
export async function POST(req: Request) {
  try {
    ...
    const enabled = await isAIFeatureEnabled(organizationId, 'ai_objection_responses');
    if (!enabled) {
    ...
    const resolved = await getResolvedPrompt(organizationId, 'task_deals_objection_responses');
