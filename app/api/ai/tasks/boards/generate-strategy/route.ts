...
export async function POST(req: Request) {
  try {
    ...
    const enabled = await isAIFeatureEnabled(organizationId, 'ai_board_generate_strategy');
    if (!enabled) {
    ...
    const resolved = await getResolvedPrompt(organizationId, 'task_boards_generate_strategy');
